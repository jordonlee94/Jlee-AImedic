...
if data.mode == 'skip' then
    ...
    notify(('Taxi en route. Estimated fare: $%s'):format(tostring(activeRide.fare)))
     ...
     SetNuiFocus(false, false)
      ...
      -- smart driving thread: obstacle avoidance (smooth) and light-ignore behavior
      CreateThread(function()
          local checkInterval = 400  -- Reduced from 600ms for faster obstacle detection
          local rayDist = 8.0
          local dodgeDist = 6.0
          ...
          while activeRide and rideRef == activeRide do
              ...
              -- simple forward raycast to detect obstacles (vehicles/peds/walls)
              ...
              if hit and DoesEntityExist(entityHit) then
                  ...
                  if dist < rayDist then
                      ...
                      if isValidRide(activeRide) then
                          ...
                          Wait(900) -- give time to start dodge
                          -- resume main drive
                          if isValidRide(activeRide) then
                              TaskVehicleDriveToCoordLongrange(driver, vehicle, dest.x, dest.y, dest.z, Config.TaxiSpeed, SMART_DRIVING_STYLE, 5.0)
                          end
      ...
      CreateThread(function()
          ...
          while activeRide and rideRef == activeRide do
              Wait(Config.AntiStuck.CheckInterval or 2000)
             ...
             lastPos = cur
              ...
              -- update HUD (throttle updates to every 1 second for performance)
              local now = GetGameTimer()
              if now - lastHudUpdate > 1000 then
